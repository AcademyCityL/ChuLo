...
class Classification(pl.LightningModule):
    ...
    """
    def __init__(self, model, lr, scheduler, label_type, chunk, num_labels, dataset_size, epochs, batch_size, output_model_name):
        super().__init__()
        self.model = model
        self.lr = lr
    ...
    def validation_step(self, batch, batch_idx, prefix='val_'):
        ...
        self.log(prefix + 'losses', loss, on_step=False, on_epoch=True, prog_bar=False, logger=True)
        self.log(prefix + 'time', time.time() - start, on_step=False, on_epoch=True, prog_bar=True, logger=True)

        return metrics

    def validation_epoch_end(self, outputs, prefix='val_', rtpl = False):
        labels = []
        predictions = []
        ...
        if rtpl == True:
            torch.save({"preds": predictions,"labels":labels},f"{output_model_name}.results")
    ...
    def test_epoch_end(self, outputs):
        self.validation_epoch_end(outputs, prefix="test_",rtpl=True)
    ...
    #     metrics = self.validation_step(batch, batch_idx, 'predict_')
    #     return metrics

    # def predict_epoch_end(self, outputs):
    #     self.validation_epoch_end(outputs, prefix="predict_")
...
if __name__ == "__main__":
    ...
    elif args.data.lower() == 'lun':
        label_type = 'multi_class'
        text_set, label_set, num_labels = dataloader.prepare_lun_data()
    else:
    ...
    task = Classification(model, args.lr, args.scheduler, label_type, chunk, num_labels, dataset_size, args.epochs,
                          args.batch_size, output_model_name)
    ...
    if not args.eval: # train mode
        ...
        for _ckpt in range(len(trainer.checkpoint_callbacks)):
            ...
            #             logging.info("long_{}_{}: {}".format(split, key, metric[key]))
                # # add by YL
                # if split == 'test':
                #     predictions_and_labels = trainer.predict(dataloaders=long_dataloaders[split], ckpt_path=ckpt_path)
                #     save_name = f'{args.model_name}_{args.data}_{args.batch_size}_{args.lr}_{args.epochs}_{args.scheduler}'
                #     torch.save(save_name, predictions_and_labels)
        ...
        for ckpt_path in ckpt_paths:
            logging.info("Checkpoint path: {}".format(ckpt_path))
            task.load_from_checkpoint(ckpt_path, model=model, lr=args.lr, scheduler=args.scheduler, label_type=label_type, num_labels=num_labels, chunk=chunk, dataset_size=dataset_size, epochs=args.epochs, batch_size= args.batch_size, output_model_name = output_model_name)
            
            # model, args.lr, args.scheduler, label_type, chunk, num_labels, dataset_size, args.epochs,
            #               args.batch_size
            # 'chunk', 'dataset_size', 'epochs', and 'batch_size'
