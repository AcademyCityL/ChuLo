...
class EurlexDataset(Dataset):
    ...
    split2folders = {'train':'train','val':'dev','test':'test'}
    def __init__(self, file_path, max_seq_len, ratio = 1, tokenizer = None, split='train',\
                 attn_mode={'name':'default','param1':0},inverse = False):
        super(EurlexDataset,self).__init__()
        # print(file_path)
    ...
    def loadFile(self,split):
        data = []
        data_file_path = self.split2folders[split]
        file_path_list = glob.glob(os.path.join(self.file_path, data_file_path, '*.json'))
        text = []
        labels = []
        ...
        if self.inverse == True:
            for file_path in file_path_list:
                with open(file_path) as file:
                    one_doc_file = json.load(file)
                one_doc_text = []
                one_doc_text.extend(one_doc_file['main_body'])
                one_doc_text.append(one_doc_file['header'])
                one_doc_text.append(one_doc_file['recitals'])
                text.append('\n'.join(one_doc_text))
                labels.append(one_doc_file['concepts'])
        else:
            for file_path in file_path_list:
                with open(file_path) as file:
                    one_doc_file = json.load(file)
...
class Eurlex(pl.LightningDataModule):
    """
    """
    def __init__(self,data_path, config, pre_cache=True, inverse = False):
        super(Eurlex,self).__init__()
    ...
    def init_datasets(self):
        for split in ['train', 'val', 'test']:
            ...
            self.datasets[split] = EurlexDataset(file_path=self.data_path,tokenizer=None,
                                max_seq_len=self.max_seq_len,ratio = ratio,split = split,attn_mode=self.attn_mode,inverse=self.inverse)
        ...
        self.valid_set = self.datasets['val']
        self.init_tokenizer()
        # preprocess
        self.preprocess()
    ...
    def test_dataloader(self):
        ...
        # for test longer documents
        test_split = torch.load("./data/eurlex/test_len_q3_idx_list.pt")
        self.test_set = data.Subset(self.datasets['test'], test_split)
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = EurlexDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = EurlexDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentEurlex(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentEurlex, self).__init__()
    ...
    def save_analysis_data(self):
        save_file_name = self.dataset_name + '_' + self.lrscheduler + '_'
    ...
    def init_metrics(self):
        ...
        for split in ['train','val','test','predict']:
            ...
            # use the same evaluation metric with https://aclanthology.org/2022.acl-short.79.pdf, note that he use micro f1
    ...
    def compute_loss(self,output,batch):
        if self.loss == 'ce': 
            ...
            loss = nn.functional.hinge_embedding_loss(output, batch['targets'].float(),reduction='mean')
        return loss