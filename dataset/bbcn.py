...
class BBCNewsDataset(Dataset):
    ...
    def _loadFile(self, split):
        try:
            dataset = datasets.load_from_disk('./data/bbcn/')
        except:
            dataset = datasets.load_dataset('SetFit/bbc-news')
            dataset.save_to_disk('./data/bbcn/')
        text = []
        labels = []
        for huggingface_split in ['train','test']:
            for item in dataset[huggingface_split]:
...
class BBCNews(pl.LightningDataModule):
    def __init__(self,data_path, config, pre_cache=True):
        super(BBCNews,self).__init__()
    ...
    def init_datasets(self):
        for split in ['train']:# 10%(65) for test
            ...
            self.datasets[split] = BBCNewsDataset(file_path=self.data_path,tokenizer=None,
        ...
        self.train_set = data.Subset(self.datasets['train'], range(0,1602))
        self.valid_set = data.Subset(self.datasets['train'], range(1602,1780))
        self.test_set = data.Subset(self.datasets['train'], range(1780,2225))
        # self.train_set, self.valid_set, self.test_set = data.random_split(self.datasets['train'], \
        #         [0.72,0.08,0.2], generator=seed) 
        # else:
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = BBCNewsDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = BBCNewsDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentBBCNews(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentBBCNews, self).__init__()
