...
class SSTDataset(Dataset):
    ...
    def loadFile(self):
        ...
        data_df = pd.read_csv(fpath,sep='\t',header=None)
        # ids = data_df['ArticleId'].tolist()
        data = data_df[0].tolist()
        labels = data_df[1].tolist()
        lenth = len(data)
        data = data_df[0].tolist()[:int(lenth)*self.ratio]
        labels = data_df[1].tolist()[:int(lenth)*self.ratio]
...
class SST(pl.LightningDataModule):
    """
    """
    ...
    def init_attr_from_config(self):
        ...
        self.vocab_name = data_config.get('vocab','results/vocabs/sst_vocab.pt')
    ...
    def init_datasets(self):
        for split in ['train', 'val', 'test']:
            ...
            self.datasets[split] = SSTDataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = SSTDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = SSTDataset.collate_fn_non_bert
        else:
...
class ExperimentSST(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentSST, self).__init__()
