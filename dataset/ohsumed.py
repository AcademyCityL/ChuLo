...
class OhsumedDataset(Dataset):
    ...
    def loadFile(self):
        data_fpath = os.path.join(self.file_path, self.file_name['data'])
        split_fpath = os.path.join(self.file_path, self.file_name['split'])
        labels = []
        data = []
        all_documents,all_splits,all_labels = [],[],[]
        c = 1
        with open(data_fpath, 'r', encoding='latin1') as f:
            for line in f.readlines():
                if c==1:
                    print(line)
                    c += 1
                all_documents.append(line)
        with open(split_fpath, 'r', encoding='latin1') as f:
            for line in f.readlines():
                _, split, label = line.split()
                all_splits.append(split)
                all_labels.append(label)
        
        for document, split, label in zip(all_documents,all_splits,all_labels):
            if self.split in split:
...
class Ohsumed(pl.LightningDataModule):
    """
    """
    def __init__(self,data_path, config, pre_cache=True):
        super(Ohsumed,self).__init__()
    ...
    def init_attr_from_config(self):
        ...
        self.vocab_name = data_config.get('vocab','results/vocabs/Ohsumed_vocab.pt')
        self.chunking = data_config.get('chunking',False)
        self.parser_type = 'stanford' if self.chunking in (33,44) else 'berkeley' #else in (11,22)


    def init_datasets(self):
        for split in ['train', 'test']:
            ratio = self.train_ratio if split == 'train' else 1
            self.datasets[split] = OhsumedDataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = OhsumedDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = OhsumedDataset.collate_fn_non_bert
        else:
...
class ExperimentOhsumed(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentOhsumed, self).__init__()
    ...
    def forward(self, batch, batch_idx,loss=True):
        ...
        if self.use_tr_tokenizer != False or self.use_chunk_emb == True:
            model_output, attn = self.model(inputs,attention_mask=batch['attention_mask'],input_ids_2=batch['input_ids_2'],\
                                      chunks_tensor = batch.get('chunked_sents',None))
        else:
            model_output, attn = self.model(inputs,attention_mask=batch['attention_mask'],chunks_tensor = batch.get('chunked_sents',None))
        if self.global_config['MODEL']['name'] != 'BERT':
    ...
    def on_train_epoch_end(self, epoch_outputs):
        ...
        for batch_outputs in epoch_outputs:
            # print('train, batch_outputs  ',batch_outputs)
    ...
    def on_validation_epoch_end(self, epoch_outputs):
        ...
        total_samples = 0
        for batch_outputs in epoch_outputs:
            # print('val, batch_outputs  ',batch_outputs)
            total_loss += batch_outputs['loss'] * batch_outputs['bs']
            total_samples += batch_outputs['bs']
    ...
    def on_test_epoch_end(self, epoch_outputs):
        ...
        total_samples = 0
        for batch_outputs in epoch_outputs:
            # print('test, batch_outputs  ',batch_outputs)
            total_loss += batch_outputs['loss'] * batch_outputs['bs']
            total_samples += batch_outputs['bs']
