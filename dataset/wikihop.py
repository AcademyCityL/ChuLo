...
class HyperParTisanDataset(Dataset):
    ...
    def _loadFile(self, split):
        ...
        text = []
        labels = []
        train_lenth = int(0.9*len(dataset['train']))
        if split == 'train':
            for i in range(0,train_lenth):
                item = dataset[split][i]
                text.append(item['text'])
                labels.append(item['hyperpartisan'])
        elif split == 'test':
            for i in range(train_lenth,len(dataset['train'])):
...
class HyperParTisan(pl.LightningDataModule):
    ...
    def init_attr_from_config(self):
        ...
        self.vocab_name = data_config.get('vocab','results/cache/vocabs/hyperpartisan_vocab.pt')
    ...
    def init_datasets(self):
        for split in ['train','test']:# 10%(65) for test
