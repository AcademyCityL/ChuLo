...
class NG20Dataset(Dataset):
    pre = 'bydate'
    group_names = ['alt.atheism','comp.graphics','comp.os.ms-windows.misc','comp.sys.ibm.pc.hardware',
    ...
    #     real_gn = pre+'_'+gn
    #     newsgroup = datasets.load_dataset('newsgroup',real_gn)
    #     train_sentences.extend(newsgroup['train'])
    #     test_sentences.extend(newsgroup['test'])
    # # print(test_sentences[0])
    # print(len(train_sentences),len(test_sentences))
    # len_stat = [len(x['text'].split()) for x in train_sentences]
    # len_stat.extend([len(x['text'].split()) for x in test_sentences])
    ...
    def loadFile(self,split):
        ...
        for gn in self.group_names:
            real_gn = self.pre+'_'+gn
            try:
                newsgroup = datasets.load_from_disk('./data/ng20/{}/'.format(real_gn))
            except:
                newsgroup = datasets.load_dataset('newsgroup',real_gn)
                newsgroup.save_to_disk('./data/ng20/{}/'.format(real_gn))
            for i in range(len(newsgroup[split])):
                text.append(newsgroup[split][i]['text'])
            labels.extend([real_gn]*len(newsgroup[split]))
        assert len(text) == len(labels),'ERROR, the lenths are different'

        # Specific process for key_phrase_extraction:
        if split == 'test':
            text[807] = text[807][:1782] + text[807][160447:] # remove the .zip file code
        return text, labels
...
class NG20(pl.LightningDataModule):
    """
    """
    def __init__(self,data_path, config, pre_cache=True):
        super(NG20,self).__init__()
    ...
    def init_datasets(self):
        for split in ['train', 'test']:
            ...
            self.datasets[split] = NG20Dataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = NG20Dataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = NG20Dataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentNG20(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentNG20, self).__init__()
