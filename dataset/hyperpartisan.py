...
class HyperParTisanDataset(Dataset):
    ...
    def _loadFile(self, split):
        # try:
        #     dataset = datasets.load_from_disk('./data/hyperpartisan/')
        # except:
        dataset = datasets.load_dataset('hyperpartisan_news_detection','byarticle')
        dataset.save_to_disk('./data/hyperpartisan/')
        text = []
        labels = []
    
        for item in dataset[split]:
            text.append(item['text'])
            labels.append(item['hyperpartisan'])

        return text, labels

    def loadFile(self,split):
        text,labels = self._loadFile(split)
...
class HyperParTisan(pl.LightningDataModule):
    ...
    def init_datasets(self):
        for split in ['train']:# 10%(65) for test
            ratio = self.train_ratio if split == 'train' else 1
            self.datasets[split] = HyperParTisanDataset(file_path=self.data_path,tokenizer=None,
                                max_seq_len=self.max_seq_len,ratio = ratio,split = split,attn_mode=self.attn_mode)
        # split as Longformer
        with open('data/hyperpartisan/hp-splits.json') as f:
            s = json.load(f)
            self.train_set = data.Subset(self.datasets['train'], s['train'])
            # self.valid_set = data.Subset(self.datasets['train'], s['dev'])
            self.valid_set = data.Subset(self.datasets['train'], s['test'])
            self.test_set = data.Subset(self.datasets['train'], s['test'])
            # >= 1454
            # new_test_split = [593, 258, 159, 411, 434, 400, 183, 134, 19, 253, 201, 221, 462, 535, 256, 555, 181]
            # self.test_set = data.Subset(self.datasets['train'], new_test_split)
        # seed = torch.Generator().manual_seed(0)
        # # if self.val_split_ratio > 0:
        # #     val_len = int(len(self.datasets['train']) * self.val_split_ratio)
        # self.train_set, self.valid_set, self.test_set = data.random_split(self.datasets['train'], \
        #         [0.8,0.1,0.1], generator=seed) # use the split in longformer paper
        # else:
    ...
    def train_dataloader(self):
        return DataLoader(self.train_set, batch_size=self.batch_size, shuffle=True,\
            num_workers=self.num_workers, collate_fn=lambda examples: \
                self.train_val_test_collect_fn(self.datasets['train'], examples),timeout=0)

    def val_dataloader(self):
        dataset = self.datasets['train']
        return DataLoader(self.valid_set, batch_size=self.batch_size, shuffle=False,\
            num_workers=self.num_workers, collate_fn=lambda examples: \
                self.train_val_test_collect_fn(dataset, examples),timeout=0)
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = HyperParTisanDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = HyperParTisanDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentHyperParTisan(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentHyperParTisan, self).__init__()
    ...
    def compute_metrics_step(self,split, preds, batch):
        ...
        microf1score = self.microf1score[split](preds.detach(), targets)
        return acc, f1score,microf1score
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds = self.s(logits)
        acc, f1_score, microf1_score = self.compute_metrics_step('train', preds, batch)
    ...
    def on_train_epoch_end(self):
        ...
        logs = {'train_loss': loss, 'train_macro_f1': macro_f1, 'train_micro_f1':microf1_score, 'train_acc': acc}
        # print(logs)
    ...
    def on_validation_epoch_end(self):
        ...
        loss = total_loss/total_samples
        acc,macro_f1,microf1_score = self.compute_metrics_epoch('val')
        # print('loss ',loss,acc)
        logs = {'val_loss': loss, 'val_macro_f1': macro_f1, 'val_micro_f1':microf1_score,'val_acc': acc}
        self.log_dict(logs,prog_bar=True)
        self.check_error()
        self.eval_results = []
        return None
    ...
    def on_test_epoch_end(self):
        ...
        loss = total_loss/total_samples
        acc,macro_f1,microf1_score = self.compute_metrics_epoch('test',)
        # print('loss ',loss,acc)
        logs = {'test_loss': loss, 'test_macro_f1': macro_f1,'test_micro_f1':microf1_score,'test_acc': acc}
        self.log_dict(logs,prog_bar=True)
