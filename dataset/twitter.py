...
from tools.textprocesser import process_tweet
...
class TwitterDataset(Dataset):
    ...
    """
    def __init__(self, max_seq_len, ratio = 1, tokenizer = None, split='train'):
        # print(file_path)
    ...
    def loadFile(self):
        ...
        length = len(all_positive_tweets)
        if self.split == 'train':
            data = all_positive_tweets[:int(0.8*length)] + all_negative_tweets[:int(0.8*length)]
            labels = np.append(np.ones((int(len(data)/2))), np.zeros((int(len(data)/2))))
        else:
            data = all_positive_tweets[int(0.8*length):] + all_negative_tweets[int(0.8*length):]
            labels = np.append(np.ones((int(len(data)/2))), np.zeros((int(len(data)/2))))
...
class Twitter(pl.LightningDataModule):
    ...
    def init_attr_from_config(self):
        ...
        self.add_sep = self.tokenizer_params.get('add_sep', False)
        ...
        self.vocab_name = data_config.get('vocab','results/vocabs/tweet_vocab.pt')
    ...
    def init_datasets(self):
        for split in ['train', 'test']:
            ...
            self.datasets[split] = TwitterDataset(tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = TwitterDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = TwitterDataset.collate_fn_non_bert
        else:
...
class ExperimentTwitter(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentTwitter, self).__init__()
    ...
    def init_attr_from_config(self):
        ...
        self.add_sep = self.tokenizer_params.get('add_sep',False) if self.tokenizer_type == 'non_bert' else True
        # ---------------------------------------------------------------- #
