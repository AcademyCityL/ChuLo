...
class MRDataset(Dataset):
    ...
    """
    file_name = {
        'train': ('text_train.txt','label_train.txt'),
        'test': ('text_test.txt','label_test.txt')
    }
    ...
    def loadFile(self):
        text = self._loadFile(os.path.join(self.file_path, self.file_name[self.split][0]))
        labels = self._loadFile(os.path.join(self.file_path, self.file_name[self.split][1]))
...
class MR(pl.LightningDataModule):
    ...
    def init_datasets(self):
        for split in ['train', 'test']:
            ...
            self.datasets[split] = MRDataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = MRDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = MRDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentMR(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentMR, self).__init__()
