...
class MASHQADataset(Dataset):
    data_files = {'train':'train_webmd_squad_v2_full.json',
                  'val':'val_webmd_squad_v2_full.json',
                  'test':'test_webmd_squad_v2_full.json',}
    ...
    def _loadFile(self, split):
        ...
        # Load the JSON data
        def load_mashqa_data(json_file_path):
            with open(json_file_path, 'r') as file:
                data = json.load(file)
            return data

              # Path to the JSON file
        json_file_path = os.path.join(self.file_path, self.data_files[split])

        # Load and convert the data
        mashqa_data = load_mashqa_data(json_file_path)

        text = []
        labels = []

        for article in mashqa_data['data']:
            for paragraph in article['paragraphs']:
                context = paragraph['context']
                text.append(context)
                # todo later
                labels.append("todo later")
...
class MASHQA(pl.LightningDataModule):
    def __init__(self,data_path, config, pre_cache=True):
        super(MASHQA,self).__init__()
    ...
    def init_datasets(self):
        for split in ['train','val','test']:# 
            ...
            self.datasets[split] = MASHQADataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = MASHQADataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = MASHQADataset.collate_fn_non_bert
        else:
