...
FILES = {
        'train':'offenseval.csv',
        'test':'offenseval.csv',
}

class OffensEvalDataset(Dataset):
    """
    ...
    def __init__(self, file_path, max_seq_len, ratio = 1, tokenizer = None, split='train'):
        ...
        self.split = split
        self.data, self.labels = self.loadFile(self.file_path)
    ...
    def loadFile(self, fpath):
        ...
        data = data_df['tweet'].tolist()
        labels = data_df['label'].tolist()
        length = len(data)
        if self.split == 'train':
            data = data[:int(0.9*length)]
            labels = labels[:int(0.9*length)]
        else:
            data = data[int(0.9*length):]
            labels = labels[int(0.9*length):]
...
class OffensEval(pl.LightningDataModule):
    """
    """
    def __init__(self,data_path, config, pre_cache=True):
        super(OffensEval,self).__init__()
    ...
    def init_datasets(self):
        ...
        for split in ['train', 'test']:
            ...
            self.datasets[split] = OffensEvalDataset(file_path=filepath, tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = OffensEvalDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = OffensEvalDataset.collate_fn_non_bert
        else:
...
class ExperimentOffensEval(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentOffensEval, self).__init__()
    ...
    def init_attr_from_config(self):
        # ---------------------------------------------------------------- #
        data_config = self.global_config['DATA']
        self.dataset_name = data_config.get('dataset_name','offenseval')
