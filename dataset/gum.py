    ...
    def _loadFile(self, split):
        ...
        def read_conllu_file(file_path):
            with open(file_path, 'r', encoding='utf-8') as file:
                data = [tokenlist for tokenlist in parse_incr(file)]
            return data

        def extract_ner_tokens(data):
            ner_tags = []
            tokens = []
            ent_record = {}
            real_pos = 0
            for sentence in data:
                ...
                for pos, token in enumerate(sentence):
                    ...
                    token_ents = []
                    cur_ent_numbs = []
                    if token.get('misc',None) is not None and 'Entity' in token['misc']:
                        ent_str_all = token['misc']['Entity']
                        b_ent_str_list = ent_str_all.split('(')[1:]
                        e_ent_numb_list = ent_str_all.split(')')[:-1]
                        for ent_str in b_ent_str_list:
                            # print("beging ent_str ",ent_str)
                            one_ent_list = ent_str.split('-')
                            ent_numb, ent_name = int(one_ent_list[0]),one_ent_list[1]
                            pref = 'B'
                            bio_ent_name = f'{pref}-{ent_name}'
                            if bio_ent_name not in token_ents: # for the nested same I/B-ents, only keep one I/B-ent as the label
                                token_ents.append(bio_ent_name)
                            ent_record[ent_numb] = [real_pos, ent_name]
                            cur_ent_numbs.append(ent_numb)
                        for ent_numb in e_ent_numb_list:
                            if '(' in ent_numb:
                                one_ent_list = ent_numb.split('(')[-1].split('-')
                                ent_numb = one_ent_list[0]
                            ent_numb = int(ent_numb)
                            if ent_numb not in ent_record:
                                continue
                            b_pos, ent_name = ent_record[ent_numb]
                            if real_pos > b_pos:
                                bio_ent_name = f'I-{ent_name}'
                                if bio_ent_name not in token_ents:
                                    token_ents.append(bio_ent_name)
                            ent_record.pop(ent_numb)

                    if len(ent_record) > 0:
                        for ent_numb, ent_rec in ent_record.items():
                            if ent_numb in cur_ent_numbs:
                                continue
                            bio_ent_name = f'I-{ent_rec[1]}'
                            if bio_ent_name not in token_ents:
                                token_ents.append(bio_ent_name)
                    if len(token_ents) == 0:
                        token_ents = ['O']
                    # print("token ents ", token_ents)
                    ner_tags.append(token_ents)
                    real_pos += 1
            return ner_tags, tokens

        def read_gum_dataset(directory):
            gum_data = {}
            for filename in os.listdir(directory):
                if filename.endswith(".conllu"):
                    file_path = os.path.join(directory, filename)
                    data = read_conllu_file(file_path)
                    ner_tags, tokens = extract_ner_tokens(data)
                    gum_data[filename.split(".conllu")[0]] = {
                        "ner_tags": ner_tags,
                        "tokens": tokens
                    }
            return gum_data

        def check(gum_dataset):
            for k,v in gum_dataset.items():
                for nertags in v['ner_tags']:
                    if len(nertags) != len(set(nertags)):
                        print("error ",nertags)

        # Directory where the GUM CoNLL-U files are stored
        gum_directory = os.path.join(self.file_path,'dep')

        # Read the dataset
        gum_dataset = read_gum_dataset(gum_directory)

        # Read the split file
        with open("data/gum/split.json") as f:
        ...
        for doc_name in split_dict[split]:
            # print(split_dict[split].keys())
            if "reddit" in doc_name:
                continue
            text.append(' '.join(gum_dataset[doc_name]['tokens']))
            ori_token_list.append(gum_dataset[doc_name]['tokens'])
            labels.append(gum_dataset[doc_name]['ner_tags'])
...
class Gum(pl.LightningDataModule):
    ...
    def init_datasets(self):
        for split in ['train','val','test']:# 
            self.datasets[split] = GumDataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = GumDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = GumDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentGUM(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentGUM, self).__init__()
    ...
    def init_metrics(self):
        ...
        for split in ['train','val','test','predict']:
            ...
            # self.__setattr__(microf1_metric_attr,torchmetrics.F1Score(task='multilabel', average = 'micro', num_labels=self.data.nclasses))
    ...
    def compute_metrics_step(self,split, preds, batch):
        ...
        preds,targets = self.collect_logits_targets(preds,targets)
        preds = preds.detach()
        preds = torch.where(preds>0.5,1,0)
        # print("preds ",preds.shape,targets.shape)
        # print("preds ",preds[0],targets[0])
        microf1score = self.microf1score[split](preds, targets)
        return microf1score,len(targets)
