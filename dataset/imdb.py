...
class IMDBDataset(Dataset):
    ...
    def _loadFile(self, split):
        try:
            dataset = datasets.load_from_disk('./data/imdb/')
        except:
...
class IMDB(pl.LightningDataModule):
    ...
    def init_datasets(self):
        for split in ['train', 'test']:
            ...
            self.datasets[split] = IMDBDataset(file_path=self.data_path,tokenizer=None,
    ...
    def init_collect_fn(self):
        if self.tokenizer_type == 'bert':
            self.train_val_test_collect_fn = IMDBDataset.collate_fn_bert
        elif self.tokenizer_type == 'non_bert':
            self.train_val_test_collect_fn = IMDBDataset.collate_fn_non_bert
        else:
            print("ERROR! {} is not supported".format(self.tokenizer_type))

class ExperimentIMDB(pl.LightningModule):
    # to complete
    '''
    ...
    def __init__(self, config):
        super(ExperimentIMDB, self).__init__()
