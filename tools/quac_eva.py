...
def _eval_fn(eval_results, min_f1=0.4):
    # modify the above original eval_fn to fit our model's output
    span_overlap_stats = Counter()
    sentence_overlap = 0.
    para_overlap = 0.
    total_qs = 0.
    ...
    followups = []
    unanswerables = []
    for did, dia_results in eval_results.items():
        good_dial = 1.
        qa_dict = dia_results['qas']
        context = dia_results['context']
        for q_str, a_and_p in qa_dict.items():
            q_idx = q_str # in each dialogue, the question str itself can be used as the id
            val_spans = a_and_p[0]
            # print("a_and_p ",a_and_p)
            val_spans = handle_cannot(val_spans)
            hf1 = leave_one_out(val_spans)
            pred_span = a_and_p[1]
    ...
    unfiltered_f1 = sum(unfiltered_f1s) / len(unfiltered_f1s)
    unanswerable_score = (sum(unanswerables) / len(unanswerables))
    metric_json = {"unfiltered_f1": unfiltered_f1, "f1": overall_f1, "HEQ": HEQ_score, "DHEQ": DHEQ_score, "unanswerable_acc": unanswerable_score}
    # if verbose:
    #     print("=======================")
    #     display_counter('Overlap Stats', span_overlap_stats, f1_stats)
    # print("=======================")
...
def eavl_quac(val_file, model_output, output_path=None, min_f1=0.4, verbose=False):
    val = json.load(open(val_file, 'r'))['data']
    preds = defaultdict(dict)
    total = 0
    val_total = 0
    for line in open(model_output, 'r'):
        if line.strip():
    ...
    metric_json = eval_fn(val, preds, verbose, min_f1)
